...
public class JPgeHotel extends JPanel implements ActionListener {
	...
	public JPgeHotel(JFInterface JFInterface) {
		...
		tHotel = new Table_Hotel(JFInterface.getPersoConn().getHotel());
		...
		if (e.getSource() == btnPlusDinfo) {
			...
			if (table.getModel() == tHotel) {
				if (s != -1) {
					...
					JOptionPane.showMessageDialog(btnPlusDinfo, "Selectionner un hotel !", "Warning",
			...
			if (table.getModel() == tHotel) {
				s = table.getSelectedRow();
				if (s == -1) {
					JOptionPane.showMessageDialog(btnModif, "Vous devez sélectionné une ligne dans le tableau !",
